...
df_metadat06 <- df_metadat05 %>% select(Bemaerkn,
                        conc_qubit_ng_uL,
...
#View(df_ddpcr03.NA_for_init_conc)
...
# or instead subset to only iterate over one species
# at time
for (ns in no_of_spsc)
  print(ns)
#  use the index number for the species in the list of species
# to get the species name
sbs_spcs <- spcabbr.q[ns]

df_ddpcr03 <- df_ddpcr03[grepl(paste0(sbs_spcs,collapse = "|"),
                               df_ddpcr03$spcabbr),]
# match the 4 species names to get the colors that
# correspond to the species
clr04 <- df_clr$col.f_spc[match(sbs_spcs,df_clr$spcNm)]
# get the sepcesi names for the subsetted species,
...
df_ddpcr03 <- df_ddpcr03[grepl(testspc.Abbr,df_ddpcr03$spcabbr),]
# also subset the qpcr data frame to only comprise the
# species that are in the 'spAbttry' vector
df_qpcr03 <- df_qpcr03[grepl(testspc.Abbr,df_qpcr03$spcabbr),]

length(unique(df_qpcr03$spcabbr))
length(unique(df_ddpcr03$spcabbr))
# subset the ddpcr data frame to only comprise the 
# the std and ntc samples, to have a data frame that 
# only has the standard 
# dilution series
df_ddpcr.std <- df_ddpcr03[grepl("Standard",df_ddpcr03$WellType),]
# and subset to only comprise the samples that are 
# eDNA samples 
df_ddpcr.e <- df_ddpcr03[grepl("Unknown",df_ddpcr03$WellType),]

# subset the qpcr data frame to only comprise the 
# the std and ntc samples, to have a data frame 
# that only has the standard 
# dilution series
df_qpcr.std <- df_qpcr03[grepl("Standard",df_qpcr03$WellType),]
# and subset to only comprise the samples that are 
# eDNA samples 
df_qpcr.e <- df_qpcr03[grepl("Unknown",df_qpcr03$WellType),]
...
wd00_wd02 <- paste0(wd00,
                    paste0("/output14_",no_sbs,
                           "_G_Guri_code_on_",sbs_spcs_cnc))
#Delete any previous versions of the output directory
...
write.csv(tibl_podd,paste0(outdir02,"/tibl_podd_",no_sbs,"_",
                           sbs_spcs_cnc,".csv"))
...
if(bSaveFigures==T){
  ggsave(plot = Figure_3, 
         ...
         # qpcrrunno and qpcrrundate
         filename = paste0(outdir02,
      "/Fig03_01_ddPCR_and_qPCR_Quantification_variance_difference",
                           ".png"),
