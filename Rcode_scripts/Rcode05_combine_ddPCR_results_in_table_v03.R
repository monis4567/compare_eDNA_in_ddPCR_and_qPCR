...
# get list of csv files with full paths
lst.fl.csv <- list.files(wd00_01, full.names = T)
#grep for the '.csv' files in the directory, and place in a list
lst_fcsv <- lst.fl.csv[grep("\\.csv",lst.fl.csv)]
...
wdin01<- paste0(wd00,"/data/MONIS6_2021_data/output02_merged_txtfiles_from_mxpro_for_MONIS6")
...
lodtable1 <- aggregate(df_qPCRm01[, "Quantitycopies"],
                       list(df_qPCRm01$snm_pltn, df_qPCRm01$WellType),
                       min)
...
#exclude all observations where
...
# but without the factor for difference between qPCR and ddPCR 
df_csvs02$evl1.qP <- df_csvs02$mcp_qPCR>(df_csvs02$qlod)
...
fnm02 <- paste0(wd00_wd06,"/Fig05_v02_compare_all_ddPCR_and_qPCR.png")
#p
if(bSaveFigures==T){
...
# now remove all log10 to 1, as these are zero, and are not needed on the plot
...
wd00_wd06_tmpf05 <- paste0(wd00_wd06,"/tmp_df_csvs05.csv")
# write a temporary file for making a reprex
write.csv(df_csvs05, wd00_wd06_tmpf05)
...
# get the unique categories for the 'eval.c1' and the 'eval.c4' 
# evaluation of eDNA levels in the 'df_csvs05' data frame
# to get a df with colors for categories
df_cl.f.evct<- df_csvs05 %>% dplyr::distinct(eval.c4,eval.c1)
...
plt_01a <- ggplot(df_csvs05, aes(smplNm, 
  ...
  # use the colors from the evaluation categories data frame
        values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_csvs05$eval.c1)),
                   0.5) ) +
...
fnm02 <- paste0(wd00_wd06,"/Fig05_v03_compare_all_ddPCR_and_qPCR.png")
...
library(grid)
#create heatmap 
gplot4 <- ggplot(data=df_mp05, aes(smplNm, sample ) ) +
  geom_tile(aes(fill = log10(vln)), colour = "white") +
  # fill gradient using the viridis color palette 'mako' and 'rocket'
  # using them interchangeably using 'mako' for all the 1-2,5-6,9-10, rows
  # and using the 'rocket' for all the 3-4,7-8,11-12 rows
  scale_fill_viridis_c(option = "rocket", name="log10\n(copies/uL)", direction =-1) +
  # scale_fill_viridis_c(option = "rocket", name="log10(molecules DNA/uL)") +
  theme_bw() +
  facet_wrap(~ spcNm, scales = "free_y", ncol = 1) +
  theme_minimal() +
  ...
  xlab("water sample") + ylab("species assay and machine")

gplot4
# set parameter to check whether to save plot or not
bSaveFigures <- T
# make a filename with path
fnm02 <- paste0(wd00_wd06,"/Fig05_v04_compare_all_ddPCR_and_qPCR.png")
#p
if(bSaveFigures==T){
  ggsave(gplot4,file=fnm02,
         #width=297*0.6,height=210*1.2,
...
colnames(df_lm.mod02) <- c("spcAbbr",
                           ...
                           "slope",
                           "RSqdRn")
write.csv(df_lm.mod02, file=paste0(wd00_wd06,"/table_w_linear_model_regression_results_v02.csv"))
...
write.csv(df_fit2, file=paste0(wd00_wd06,"/table_w_linear_model_regression_results_v03.csv"))
...
#Write out the table
write.csv(df_fit2, file=paste0(wd00_wd06,"/table_w_linear_model_regression_results_v04.csv"))
...
#p2
outF.Nm_plt <- paste0(wd00_wd06,"/","Fig06_v02_regr_ddPCR_qPCR.png")
ggsave(p2,filename=outF.Nm_plt,
...
# see the ggplot object
#gplot4a
outF.Nm_plt <- paste0(wd00_wd06,"/","Fig07_v02_ddPCR_qPCR_1a.png")
ggsave(gplot4a,filename=outF.Nm_plt,
...
#gplot4b
outF.Nm_plt <- paste0(wd00_wd06,"/","Fig08_v02_ddPCR_qPCR_1b.png")
ggsave(gplot4b,filename=outF.Nm_plt,units="cm",
...
# see the ggplot object
#gplot4c
outF.Nm_plt <- paste0(wd00_wd06,"/","Fig08_v02_ddPCR_qPCR_1c.png")
ggsave(gplot4c,filename=outF.Nm_plt,units="cm",
       width=12,height=12,dpi=300,scale=2)
#
...
# see plot
#gplot4d
outF.Nm_plt <- paste0(wd00_wd06,"/","Fig08_v02_ddPCR_qPCR_2a.png")
ggsave(gplot4d,filename=outF.Nm_plt,units="cm",
       width=12,height=12,dpi=300,scale=2)
...
# see the plot
#gplot4d
outF.Nm_plt <- paste0(wd00_wd06,"/","Fig08_v02_ddPCR_qPCR_2b.png")
...
extwd00 <- paste0(wd00,"/data/","MONIS6_2021_data")
...
# write out the 'df_csvs05.2' data frame as a '.csv' file
outFlNm.csv <- paste0(wd00_wd06,"/","df_csvs05.2_v01.csv")
write.table(df_csvs05.2, file=outFlNm.csv, row.names = FALSE, sep = ",")
...
p05 <- ggplot(data = sf.df.world) +
  ...
  scale_fill_manual(
    name = 'evaluation',
    # use the colors from the evaluation categories data frame
    values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_csvs05.2s$eval.c1)),
                   0.5) ) +
...
#see the plot
#p05t
outFlNm.fig09 <- paste0(wd00_wd06,"/","Fig09_v02_map_spring_ddPCR_qPCR_1b.png")
# save the plot
ggsave(p05t,filename=outFlNm.fig09,
       units="mm",
...
p05 <- ggplot(data = sf.df.world) +
  ...
  scale_fill_manual(
    name = 'evaluation',
    # use the colors from the evaluation categories data frame
    values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_csvs05.2f$eval.c1)),
                   0.5) ) +
  #change axis labels
  xlab("longitude") + ylab("latitude") +
  #xlab("længdegrad") + ylab("breddegrad") +
...
#getwd()
outFlNm.fig09 <- paste0(wd00_wd06,"/","Fig09_v02_map_fall_ddPCR_qPCR_1b.png")
# save the plot
ggsave(p05t,filename=outFlNm.fig09,
       units="mm",
       width=210,height=297,dpi=300,scale=1.4)
...
p06 <- ggplot(data = sf.df.world) +
  ...
  scale_fill_manual(
    name = 'evaluation',
    # use the colors from the evaluation categories data frame
    values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_csvs05.1s$eval.c1)),
                   0.5) ) +
...
#p06t
#getwd()
outFlNm.fig09 <- paste0(wd00_wd06,"/","Fig09_v02_map_spring_ddPCR_qPCR_2b.png")
# save the plot
ggsave(p06t,filename=outFlNm.fig09,
       units="mm",
       width=210,height=297,dpi=300,scale=1.4)
...
p06 <- ggplot(data = sf.df.world) +
  ...
  scale_fill_manual(
    name = 'evaluation',
    # use the colors from the evaluation categories data frame
    values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_csvs05.1f$eval.c1)),
                   0.5) ) +
...
p06t <- p06
#getwd()
outFlNm.fig09 <- paste0(wd00_wd06,"/","Fig09_v02_map_fall_ddPCR_qPCR_2b.png")
# save the plot
ggsave(p06t,filename=outFlNm.fig09,
       units="mm",
       width=210,height=297,dpi=300,scale=1.4)
...
p07 <- ggplot(data = sf.df.world) +
  ...
  scale_fill_manual(
    name = 'evaluation',
    # use the colors from the evaluation categories data frame
    values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_csvs05.1c01$eval.c1)),
                   0.5) ) +
...
#getwd()
outFlNm.fig10 <- paste0(wd00_wd06,"/","Fig10_v02_map_seasons_ddPCR_qPCR_3a.png")
# save the plot
ggsave(p07t,filename=outFlNm.fig10,
       units="mm",
...
p07 <- ggplot(data = sf.df.world) +
  ...
  scale_fill_manual(
    name = 'evaluation',
    # use the colors from the evaluation categories data frame
    values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_csvs05.1c02$eval.c1)),
                   0.5) ) +
  # scale_fill_manual(values=alpha(c("red","black","orange","yellow","white"),0.5),
  #                   name="evaluation") +
...
outFlNm.fig10 <- paste0(wd00_wd06,"/","Fig10_v02_map_seasons_ddPCR_qPCR_3b.png")
# save the plot
ggsave(p07t,filename=outFlNm.fig10,
       units="mm",
       width=210*0.8,height=297*0.6,dpi=300,scale=1.4)
...
#_______________________________________________________________________________
# 03 -start - Make a heatmap of ddPCR and qPCR results
#_______________________________________________________________________________
library(ggplot2)
library(dplyr)
library(patchwork)
...
# Make all columns with numbers numeric in 'df_mp05' dataframe
df_mp05 <- df_mp05 %>% mutate(across(where(is.character), as.numeric))

# Copy the data frame to have the original data frame intact
df_mp06 <- df_mp05

# Ensure 'vln' is a double
df_mp06$vln <- as.double(df_mp06$vln)

# Replace 'Cragig' with 'Maggig' in the 'spcNm' column
df_mp06$spcNm <- gsub("Cragig", "Maggig", df_mp06$spcNm)

# Left join with df_clr2 using 'spcNm' as the common column
df_mp06 <- dplyr::left_join(df_mp06, df_clr2, by = "spcNm")

# Create 'colr' and 'sample' columns
df_mp06$colr <- df_mp06$spcNm
df_mp06$sample <- paste0(df_mp06$spcNm, "_", df_mp06$machine)
# Filter out zero values for log10 transformation
positive_x <- df_mp06 %>% filter(vln > 0)  # Only include positive values of x

# Compute the adjusted minimum and maximum for the log10 scale
min_log_x <- min(log10(positive_x$vln))  # Min of log10(x) for positive x
max_log_x <- max(log10(positive_x$vln))  # Max log10(x)

# Define the color palette for species names colors
un_Nams <- sort(unique(df_clr2$spcNm))  # Sort species names
un_Nams <- factor(un_Nams, levels = un_Nams, ordered = TRUE)  # Convert to ordered factor
pl_colss <- df_clr2$col.f_spc
names(pl_colss) <- un_Nams
# make a another data frame that holds colors and long species names
un_NamsL <- sort(unique(df_clr2$shrtNm))  # Sort species names
un_NamsL <- factor(un_NamsL, levels = un_NamsL, ordered = TRUE)  # Convert to ordered factor
pl_colssL <- df_clr2$col.f_spc
names(pl_colssL) <- un_NamsL

# Function to create each individual plot
make_facet_plot <- function(color_label, color_hex, show_x_axis = FALSE) {
  subset_di <- df_mp06 %>% filter(colr == color_label)
  
  ggplot(data = subset_di, aes(smplNm, sample, fill = log10(vln))) +
    ...
    theme(
      ...
      axis.text.x = if (show_x_axis) {
        element_text(angle = 90, vjust = 0.5, hjust = 1)  # Rotate x-axis labels by 90 degrees
      } else {
        element_blank()
      },
      ...
      strip.text.y.left = element_text(size = 14, face = "italic", angle = 0, vjust = 0.5),  # Rotate the facet labels for y-axis
      strip.text.x = element_text(size = 8,  angle = 0, vjust = 0.5)  # Optionally rotate facet labels on x-axis (for clarity)
    ) +
    scale_y_discrete(limits = rev) +
...
# Create all plots, enabling x-axis only on the last ("Psevar")
plot_list <- lapply(un_Nams, function(col) {
  make_facet_plot(col, pl_colss[col], show_x_axis = col == "Psever")
})
...
dummy_plot <- ggplot(data = df_mp06, aes(smplNm, sample, fill = log10(vln))) +
  geom_tile(colour = "white") +
  scale_fill_gradientn(
...
bSaveFigures <- T
# make a filename with path
fnm02 <- paste0(wd00_wd06,"/Fig05_v05_compare_all_ddPCR_and_qPCR.png")
#p
if(bSaveFigures==T){
  ggsave(final_plot,file=fnm02,
         #width=297*0.6,height=210*1.2,
         height=297*0.7,width=210,
         units="mm",dpi=300)
}
#View(df_csvs01)
#___
...
# check which colors are associated with which species
df_mp06 %>% dplyr::select(shrtNm,col.f_spc) %>% distinct()
df_mp06 %>% dplyr::select(shrtNm,col.f_spc,spcNm) %>% distinct()
# Create 'colr' and 'sample' columns
df_mp06$colr <- df_mp06$shrtNm
df_mp06$sample <- paste0(df_mp06$spcNm, "_", df_mp06$machine)
# Filter out zero values for log10 transformation
positive_x <- df_mp06 %>% filter(vln > 0)  # Only include positive values of x
...
# Define the color palette for species names colors
un_Nams <- sort(unique(df_mp06$spcNm))  # Sort species names
un_Nams <- factor(un_Nams, levels = un_Nams, ordered = TRUE)  # Convert to ordered factor
pl_colss <- df_clr2$col.f_spc
names(pl_colss) <- un_Nams
...
# make a another data frame that holds colors and long species names
un_NamsL <- sort(unique(df_mp06$shrtNm))  # Sort species names
un_NamsL <- factor(un_NamsL, levels = un_NamsL, ordered = TRUE)  # Convert to ordered factor
pl_colssL <- df_clr2$col.f_spc[match(un_NamsL,df_clr2$shrtNm)]
names(pl_colssL) <- un_NamsL
...
# Create all plots, enabling x-axis only on the last ("P. verruculosa")
plot_list <- lapply(un_NamsL, function(col) {
  make_facet_plot(col, pl_colssL[col], show_x_axis = col == "P. verruculosa")
})
# Combine using patchwork with equal height for each subplot
combined_plot <- wrap_plots(plotlist = plot_list, ncol = 1) &
  theme(plot.margin = margin(0, 0, 0, 0))  # Ensure no extra margin globally
...
bSaveFigures <- T
# make a filename with path
fnm02 <- paste0(wd00_wd06,"/Fig05_v06_compare_all_ddPCR_and_qPCR.png")
#p
if(bSaveFigures==T){
  ggsave(final_plot,file=fnm02,
         #width=297*0.6,height=210*1.2,
...
df_mp06$smplNm
df_mp06$sample
df_mp06$lngNm.mc<- paste0(df_mp06$shrtNm,"_",df_mp06$machine)
library(grid)
#create heatmap 
gplot4 <- ggplot(data=df_mp06, aes(smplNm, lngNm.mc ) ) +
  geom_tile(aes(fill = log10(vln)), colour = "white") +
  # fill gradient using the viridis color palette 'mako' and 'rocket'
  # using them interchangeably using 'mako' for all the 1-2,5-6,9-10, rows
  # and using the 'rocket' for all the 3-4,7-8,11-12 rows
  ...
  theme_minimal() +
  #facet_wrap(~ spcNm, scales = "free_y", ncol = 1) +
  facet_wrap(~ shrtNm, scales = "free_y", ncol = 1, strip.position = "left") +
  theme(plot.margin = margin(0, 0, 0, 0)) +  # No extra margin around plots)
  #theme(strip.text = element_blank()) +# Remove facet labels
  scale_y_discrete(limits=rev) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(panel.spacing = unit(0.02, "lines")) +
  
  theme(axis.text.y = element_blank() )+  # Remove the y-axis text (sample labels)
  theme(axis.ticks.y = element_blank()) +  # Optional: Remove the y-axis ticks
  theme(strip.text.y.left = element_text(size = 11,
                                         face = "italic", 
                                         hjust = -0.01,
                                         angle = 0, vjust = 0.5)) +  # Rotate the facet labels for y-axis
  xlab("water sample") + ylab("species detected")

gplot4
# set parameter to check whether to save plot or not
bSaveFigures <- T
# make a filename with path
fnm02 <- paste0(wd00_wd06,"/Fig05_v07_compare_all_ddPCR_and_qPCR.png")
#p
if(bSaveFigures==T){
  ggsave(gplot4,file=fnm02,
         #width=297*0.6,height=210*1.2,
...
# copy the labels vector, and modify it with gsub
arter_labels2 <- arter_labels
arter_labels2 <- gsub("Cragig", "Maggig", arter_labels2)
# get the long name from the 'df_clr2' data frame 
arter_labels2 <- df_clr2$shrtNm[match(arter_labels2,df_clr2$spcNm)]
## ensure the plot library is loaded
library(ggplot2)
df_mp06$shrtNm

df_mp06$sample.lnNm <- paste0(df_mp06$shrtNm, "_", df_mp06$machine)
# select  and get distinct for 'sample.lnNm'
arter_breaks2 <- 
  df_mp06 %>% select(sample.lnNm) %>% distinct() 

arter_breaks2 <- as.factor(unlist(arter_breaks2))[grepl("ddPCR",as.factor(unlist(arter_breaks2)))]
# begin the plot
plt_01a <- ggplot(df_mp06, aes(smplNm, 
                               sample.lnNm, 
                               size = eDN.lv1.l10,
                               color = machine, 
                               fill = eval.c1)) +
  ...
  scale_fill_manual(
    name = 'evaluation',
    # use the colors from the evaluation categories data frame
    values = alpha(setNames(df_cl.f.evct$eval.c4, unique(df_mp06$eval.c1)),
                   0.5) ) +
  # scale_fill_manual(values=alpha(
  # c("red","black","orange","yellow","white")
  # ,0.5),name="evaluation") +
  scale_size(range = c(0.001, 10), 
             name="log10(molecules\n DNA/uL)") +
  scale_y_discrete(name="species assay", 
                   breaks=arter_breaks2, labels=arter_labels2) +
  scale_x_discrete(name="watersample") +
  coord_flip() +
  labs(color='machine') +
  theme(axis.text.x = element_text(angle = 90, 
...
plt_01a
#make a file to save to
fnm02 <- paste0(wd00_wd06,"/Fig05_v08_compare_all_ddPCR_and_qPCR.png")
#evaluate if the plot should be saved
if(bSaveFigures==T){
  ggsave(plt_01a,file=fnm02,
         #width=297*0.6,height=210*1.2,
...
#_______________________________________________________________________________
# 03 -end - Make a heatmap of ddPCR and qPCR results
#_______________________________________________________________________________
