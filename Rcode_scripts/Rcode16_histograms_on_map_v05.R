...
create_plot <- function(data_subset, mxuch, y_max) {
  p_main <- ggplot() +
    geom_sf(data = world, colour = "black", fill = "grey71", 
            alpha = 1.0) +
    geom_sf(data = sites, colour = "black", 
            shape = 21, fill = alpha("white", 0.1),
            size = 2) +
...
outfl <- paste0(wd00_wd16, "/Fig19_combined_plots_16.png")
