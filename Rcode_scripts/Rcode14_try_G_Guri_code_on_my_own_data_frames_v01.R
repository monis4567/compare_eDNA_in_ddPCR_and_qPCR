...
# exclude rows in df_ddpcr.std if the column 'conc_copies_per_uL' has 0 or NA
#df_ddpcr.std <- df_ddpcr.std %>% dplyr::filter(conc_copies_per_uL != 0)
#df_ddpcr.std <- df_ddpcr.std %>% dplyr::filter(!is.na(conc_copies_per_uL))
# and subset to only comprise the samples that are 
# eDNA samples 
df_ddpcr.e <- df_ddpcr04[grepl("Unknown",df_ddpcr04$WellType),]
# exclude rows in df_ddpcr.e if the column 'conc_copies_per_uL' has 0 or NA
#df_ddpcr.e <- df_ddpcr.e %>% dplyr::filter(conc_copies_per_uL != 0)
#df_ddpcr.e <- df_ddpcr.e %>% dplyr::filter(!is.na(conc_copies_per_uL))
...
# exclude rows in df_qpcr.std if the column 'Ct' has 0
#df_qpcr.std <- df_qpcr.std %>% dplyr::filter(Ct != 0)
# and subset to only comprise the samples that are 
# eDNA samples 
df_qpcr.e <- df_qpcr04[grepl("Unknown",df_qpcr04$WellType),]
#exclude the species in df_qpcr.e if the column 'Ct' has 0
#Check for each species if the "df_qpcr.e$Ct" only contains
# 0 values, by getting the sum of all 'df_qpcr.e$Ct' for each species
# and then place the species name and the sum of all the Ct values per species in a 
# data frame
df_qpcr.e.ctEvl <- df_qpcr.e %>% 
  dplyr::group_by(spcabbr) %>%
  dplyr::summarise(sum_Ct = sum(Ct,na.rm = T))
...
df_ddpcr.std %>% 
  dplyr::group_by(spcabbr) %>%
  dplyr::summarise(sum_Ct = sum(Conc,na.rm = T))

df_ddpcr.e %>% 
  dplyr::group_by(spcabbr) %>%
  dplyr::summarise(sum_Ct = sum(Conc,na.rm = T))

# identify the species that has only 0 values in the 'sum_Ct' column
spc.omit <- df_qpcr.e.ctEvl %>% 
  dplyr::filter(sum_Ct == 0) %>%
  dplyr::select(spcabbr)
# convert the 'spc.omit' to a vector
spc.omit <- as.character(spc.omit$spcabbr)
# exclude these 'spc.omit' from the  
# "df_qpcr.e","df_qpcr.std","df_ddpcr.e","df_ddpcr.std" data frames
# as it is not possible to run the stanMod on the data frames
# if the species has only 0 values in the 'Ct' column
df_qpcr.e <- df_qpcr.e[!df_qpcr.e$spcabbr %in% spc.omit,]
df_qpcr.std <- df_qpcr.std[!df_qpcr.std$spcabbr %in% spc.omit,]
df_ddpcr.e <- df_ddpcr.e[!df_ddpcr.e$spcabbr %in% spc.omit,]
df_ddpcr.std <- df_ddpcr.std[!df_ddpcr.std$spcabbr %in% spc.omit,]
# check the number of unique species in the 'spcabbr' column
# in the ddpcr data frame
unique(df_ddpcr.std$spcabbr)

# Issue 1:
#   The data that is going through the model doesn't have any Ct values. At line 294: 
# > df_qpcr.e$Ct
...
# 
# If there are no observed Ct values for the environmental samples there can't be any inference on the concentration of samples.
...
# exclude the spc.omit from the sbs_spcs
s <- sbs_spcs[!sbs_spcs %in% spc.omit]
sbs_spcs <- s
...
pp1 <-
  tibl_podd %>%
  ggplot()+
  geom_line(aes(x=10^C,y=prob_dd,color=Species),lty=2,linewidth=0.9)+
  geom_line(aes(x=10^C,y=prob_q,color=Species),lty=1,linewidth=0.9)+
...
if(bSaveFigures.l3==T){
  ggsave(plot = pp2.l3, 
         ...
         # qpcrrunno and qpcrrundate
         filename = paste0(outdir02,
                           "/Fig02_02_less_than_3_ddPCR_and_qPCR_modelled_concentration",
                           no_sbs,"_",sbs_spcs_cnc,
                           ".png"),
         width=210*1.6,height=297*0.6,
         #width=297,height=210,
...
if(bSaveFigures.m3==T){
  ggsave(plot = pp2.m3, 
         ...
         # qpcrrunno and qpcrrundate
         filename = paste0(outdir02,
                           "/Fig02_02_more_than_3_ddPCR_and_qPCR_modelled_concentration",
                           no_sbs,"_",sbs_spcs_cnc,
                           ".png"),
         width=210*1.6,height=297*0.6,
         #width=297,height=210,
...
if(bSaveFigures==T){
  ggsave(plot = Figure_2, 
         ...
         # qpcrrunno and qpcrrundate
         filename = paste0(outdir02,
                           "/Fig02_03_ddPCR_and_qPCR_modelled_concentration",
                           no_sbs,"_",sbs_spcs_cnc,
                           ".png"),
...
#qe <- extract_param(stanMod_M3,"C_qPCR_joint")
#colnames(qe)
#colnames(e_qpcr)
q_bin <-
...
# #::
# exCqj <- extract_param(stanMod_M3,"C_qPCR_joint")
# colnames(exCqj)
# # 3.5 Data for Figure 3
# q_bin <-
#   #ecqj %>%
