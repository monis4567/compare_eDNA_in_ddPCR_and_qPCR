...
for (spec.lat in unq.spc_n) {
  ...
  #Sys.sleep(23)
  #}
  # set a variable to test if any plot was made
  any_plot_made <- FALSE
      ...
      for (mch_tp in mchns){
        ...
        if (nrow(df_e15_ssn) >= 3) {
          ...
          res.ipdw3 <- tryCatch({
            ...
            # Create an empty raster with the same extent and resolution as costras
            empty_raster <- raster::raster(extent = raster::extent(costras), res = raster::res(costras))
            raster::values(empty_raster) <- NA
            empty_raster
          })
        ...
        # After your plot code (inside the season/machine loop):
        any_plot_made <- TRUE
...
# if any plot is made
  if (any_plot_made) {
    mtext(
      c(paste("Fig13_v",TwN,"_Appendix E", no.spc.app.plot, "."),  sep = ""),
      outer = TRUE,
