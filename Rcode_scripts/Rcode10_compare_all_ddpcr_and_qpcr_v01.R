...
# install "sfheaders" and "geojsonsf", as they are required for   'giscoR'
...
wdout <- "output11_compare_all_ddpcr_and_qpcr"
...
#grep for the '.csv' files in the input directory, and place in a list
...
df_qPCRm02.std$mxrpl <- df_qPCRm02.mxrpl$n[match(df_qPCRm02.std$spAb_plN,
                                                 df_qPCRm02.mxrpl$spAb_plN)]
...
df_e02 <- df_e02.1
# View(df_e02.1)
# find the column number for the column with the 'copies per uL'
idxclcp <- which(grepl("\\(copies/.*L\\)",colnames(df_ddP)))

colnames(df_ddP)[idxclcp] <- "Conc.copies.uL."

#make copy count
df_ddP$Conc.copies.uL. <- as.numeric(df_ddP$Conc.copies.uL.)
# replace NAs with zeroes
df_ddP$Conc.copies.uL.[is.na(df_ddP$Conc.copies.uL.)] <- 0
#make a column with log10 to copies plus 1
df_ddP$log10.Conc.copies.µL <- log10(df_ddP$Conc.copies.uL.+1)
df_ddP$l10cp <-  df_ddP$log10.Conc.copies.µL 
# calculate the total copy number for the total reaction volumne
#df_ddP$Conc.copies.uL.[df_ddP$stdlvl!=0]
df_ddP$totcpin25uLrxn <- df_ddP$Conc.copies.uL.*25
# I used 5 uL of template so the confidence intervals should be multiplied with 5
df_ddP$ccpuL <- df_ddP$Conc.copies.uL.*5
# then the column 'Conc.copies.uL.' should 
df_ddP$ddcpuL <-df_ddP$Conc.copies.uL.
...
# or just use the unmodified copy counts per uL
df_e02$cppuLm <- df_e02$cppuL
...
df_e03 <- df_e02 %>%
  ...
  dplyr::summarise(dplyr::across(starts_with("cppuL"), list(mean = mean,
...
df_e04 <- df_e04 %>% 
  dplyr::mutate_at(c('dP.ccp','dP.pcmn','dP.pcmx'), ~replace_na(.,0))
# remove a column
...
spcabbr.unq <- sps
spcabbr.unq <- spcabbr.unq %>% sort()
spc.t.excl<- c("Neomel","Rhihar","Oncgor")
# exclude species that are not in the 'df_e04' data frame from the
# 'spcabbr.unq'
spcabbr.unq <- spcabbr.unq[!spcabbr.unq %in% spc.t.excl]
...
# find the 'spcNms' not in the 'df_clr2' data frame
no.clr2<- spcNms[!spcNms %in% df_clr$spcNm]
# use the polychrome pallette
clr05 <- palette.colors(palette = "Polychrome")
# get colors from this 'clr05' palette to assign to the 'no.clr2' species
clr05.sp <- clr05[1:length(no.clr2)]
...
plt07 <- ggplot(data=df_e04c, aes(x=smplNm,y=mlcnt )) +
  ...
  geom_tile(aes(height=Inf,fill=as.factor(smplNm),y=1), 
            position=position_nudge(x=-0.05), show.legend=F) +
  scale_fill_manual(values=alpha(clfH,0.12)) +
...
fgNm07 <- paste0(wd00,"/",wdout,"/Fig11_scatterplot_in_colmns_v03.png")
...
plt07 <- ggplot(data=df_e04c, aes(x=smplNm,y=mlcnt )) +
  ...
  xlab("sample number") +
  annotation_logticks(sides="l") +
  facet_wrap(vars(speciesNm ), ncol = 3) +
...
#make filename to save plot to save the plot to
fgNm07 <- paste0(wd00,"/",wdout,"/Fig11_scatterplot_in_colmns_v04.png")
# save the plot as png file if 'bSaveFigures' is set to be TRUE
bSaveFigures <- TRUE
if(bSaveFigures==T){
  ggsave(plt07,file=fgNm07,
...
#make filename to save plot to save the plot to
fgNm07 <- paste0(wd00,"/",wdout,"/Fig11_scatterplot_in_colmns_v05.png")
# save the plot as png file if 'bSaveFigures' is set to be TRUE
bSaveFigures <- TRUE
if(bSaveFigures==T){
  ggsave(plt07,file=fgNm07,
...
#_______________________________________________________________________________
  ...
  fgNm08 <- paste0(wd00,"/",wdout,"/Fig12_v01_scatterplot.png")
  # save the plot as png file if 'bSaveFigures' is set to be TRUE
  bSaveFigures <- TRUE
  if(bSaveFigures==T){
  ...
  fgNm08 <- paste0(wd00,"/",wdout,"/Fig12_v02_scatterplot.png")
  # save the plot as png file if 'bSaveFigures' is set to be TRUE
  bSaveFigures <- TRUE
  if(bSaveFigures==T){
    ggsave(plt08,file=fgNm08,
  ...
  fgNm08 <- paste0(wd00,"/",wdout,"/Fig12_v03_scatterplot.png")
  # save the plot as png file if 'bSaveFigures' is set to be TRUE
  bSaveFigures <- TRUE
  if(bSaveFigures==T){
    ggsave(plt08,file=fgNm08,
  ...
  #_
  #identify LOD
  lod.id.df<-sbs_csvs03[(sbs_csvs03$smplTp=='STD'),]
  
  #test if the LOD for qpcr is infinite - in case of no standard curve
  if (is.finite(min(lod.id.df$mcp_qPCR))==F) {
    print("no_std_crv")
    qlod.val <- 1
  } else {
    qlodVal <- lod.id.df$mcp_qPCR[(lod.id.df$mcp_qPCR>0)]
    qlod.val<-min(qlodVal)
    #print(lod.val)
  }
  
  #test if the LOD for ddpcr is infinite - in case of no standard curve
  if (is.finite(min(lod.id.df$mcp_ddPCR))==F) {
    print("no_std_crv")
    ddlod.val <- 1
  } else {
    dlodVal <- lod.id.df$mcp_ddPCR[(lod.id.df$mcp_ddPCR>0)]
    ddlod.val<-min(dlodVal)
    #print(lod.val)
  }
  ...
  intc3 <- round(intc2,3)
  #use the slope to calculate the efficiency
  ...
  intc4<-slo2*(log10(qlod.val))+intc2
  intc5 <- round(intc4,3)
  #mx.Cq.lod<- max(na.omit(sbs_csvs03$CtdRn[qlod.val==sbs_csvs03$mcp_qPCR]))
...
write.csv(df_lm.mod02, 
          file=paste0(wd00,"/",wdout,"/table_w_linear_model_regression_results_v06.csv"))
...
write.csv(df_fit2, file=paste0(wd00,"/",wdout,
...
dfm01 <- df %>% dplyr::select(speciesabbr,
                              ddlod,
                              qlod,
                              qloq,
                              qPCR,
...
#Write out the table
write.csv(df_fit2, file=paste0(wd00,"/",wdout,"/table_w_linear_model_regression_results_v08.csv"))
...
library(giscoR)
if(exists("DK_map")==F){
DK_map <- giscoR::gisco_get_countries(country = c("Denmark",
                                          "Sweden",
                                          "Norway",
                                          "Germany",
                                          "Poland"
...
filNm.e09 <- paste0(wd00,"/",wdout,"/table_w_ddpcr_and_qpcr_data_and_lat_lon_v01.csv")
write_csv(df_e09,filNm.e09)
...
# use left_join to append 'df_dta' to the 'df_e10' data frame
df_e10 <- df_e10 %>% dplyr::left_join(df_dta,
                                      by=c("speciesabbr"="AbbrvNm"))
# us the 'df_clr2' data frame as the 'df_clr6' data frame
df_clr6 <- df_clr2 
...
pl07 <- ggplot() +
  ...
  #scale_fill_manual()
  scale_fill_manual(values = setNames(df_clr6$col.f_spc,
                                      df_clr6$shrtNm)) +
  ...
  # increase the size of the point symbols in the legend key
  ...
  #ylim(54.2, 58) +
  # adjust the y-axis  
  scale_y_continuous(limits = c(54.2, 58), breaks = seq(54, 58, by = 1)) +
  xlab("longitude") + ylab("latitude")

pl07

fgNm08 <- paste0(wd00,"/",wdout,"/Fig13_v01_map_ddPCR_qPCR.png")
# save the plot as png file if 'bSaveFigures' is set to be TRUE
bSaveFigures <- TRUE
if(bSaveFigures==T){
...
pl07
fgNm08 <- paste0(wd00,"/",wdout,"/Fig13_v02_map_ddPCR_qPCR.png")
# save the plot as png file if 'bSaveFigures' is set to be TRUE
bSaveFigures <- TRUE
if(bSaveFigures==T){
  ggsave(pl07,file=fgNm08,
...
# make column that is common between the two data frames
df_e10$MST.n_Dinds <- paste(df_e10$U_Pr_Nr,
                            df_e10$Dato_inds,
                            sep="_")
# join the data frames using the column that is common
df_e10 <- dplyr::left_join(df_e10,df_MSTsmpls,
                   by=c("MST.n_Dinds"))
# make column that is common between the two data frames
df_e07$MST.n_Dinds <- paste(df_e07$U_Pr_Nr,
                            df_e07$Dato_inds,
                            sep="_")
# join the data frames using the column that is common
df_e11 <- dplyr::left_join(df_e10,
                           df_e07,
                           by=c("MST.n_Dinds",
                                "speciesabbr"))
...
#_______________________________________________________________________
# paste together a path for where the xls input files are located
wddata <- "data/MONIS6_2021_data"
wd00_wddata <- paste0(wd00,"/",
                      wddata)
...
lst_xlsfl_in_wd01 <- list.files(wd00_wddata)[grepl("xls",list.files(wd00_wddata))]
...
df_cex01$MST.nummer[grep("^[0-9]+{2}$",
                         df_cex01$MST.nummer)] <- paste("MST-",
...
# exclude rows with only 'MST' in the MST number
df_cex02 <- df_cex01[(df_cex01$MST.nummer!="MST"),]
# substitute the '-' to get MST numbers that are matching between data frames
df_cex02$MSTn2 <- gsub("-","",df_cex02$MST.nummer)
# join the data frames using the column that is common
df_e12 <- dplyr::left_join(df_e11,df_cex02,
                 by=c("U_Pr_Nr"="MSTn2"))
#View(df_e12)
# write out the data frame as a csv file
write.csv(df_e12,
...
# Uncomment section below to try and iterate over individual species
# #_______________________________________________________________________
