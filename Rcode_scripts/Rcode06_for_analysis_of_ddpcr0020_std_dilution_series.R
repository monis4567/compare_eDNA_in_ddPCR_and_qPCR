...
wd07 <- "output07_analysis_of_ddpcr0020"
...
inf03 <- "plate_setup_ddpcr0020_stddiltest.xls"
inf03 <- paste0(wd00_wd01,"/",inf03)
...
# or just use the original copy count per uL in the qPCR
# instead of the factor-multiplied copy count per uL
df_e02$cppuLm <- df_e02$cppuL
# modify the copy count in the qPCR 
#df_e02$cppuLm <- df_e02$cppuL
#df_e02$cppuLm <- df_e02$cppuLm
#df_e02$cppuLm <- df_e02$cppuL
...
#df_e04$dP.ccp <- df_ddP$ccpuL[match(df_e04$qP.smpltp,df_ddP$smpl.No)]
df_e04$dP.ccp <- df_ddP$ccpuL[match(df_e04$sA_sN,df_ddP$sA_sN)]

#df_e04[is.na(df_e04)] <- 0
...
ncl <- length(clfH)
clfH[(ncl-9):(ncl)] <- rep(c("orchid4","orchid1"),9)
...
plt07 <- ggplot(data=df_e05, aes(x=smplNm,y=mcp)) +
  theme_bw() +
  theme_classic() +
...
geom_tile(aes(height=Inf,fill=as.factor(smplNm),y=1), 
  ...
  ylab("measured conc (molecules/uL)") + xlab("expected conc (molecules/uL)") +
  annotation_logticks(sides="l") +
  #coord_flip()+
  scale_color_manual(values=c("springgreen4", "#E69F00"))
#plt07  <- plt07 + labs(title = "standard fortyndings række i ddPCR og qPCR")#,
plt07  <- plt07 + labs(title = "standard dilution series in ddPCR and qPCR")#,
...
inf02_1 <- gsub(".*/(.*)","\\1",inf02_1)
#make filename to save plot to save the plot to
fgNm07 <- paste0(wd00_wd07,"/Fig02_v01",inf02_1,".png")
...
labs(title = "comparison between ddPCR and qPCR")#

#plt08

fgNm08 <- paste0(wd00_wd07,"/Fig02_v02",inf02_1,".png")
# save the plot as png file if 'bSaveFigures' is set to be TRUE
bSaveFigures <- TRUE
if(bSaveFigures==T){
...
fnm02 <- paste0(wd00_wd07,"/Fig0020_",inf02_1,".png")
...
p <-  plt07t  +
  ...
  plot_layout(nrow=2,
              ncol=1,byrow=T) + #xlab(xlabel) +
  plot_layout(guides = "collect") +
  #patchwork::plot_annotation(caption="Fig0020_v01_stddilcomp") #& theme(legend.position = "bottom")
patchwork::plot_annotation(caption="") #& theme(legend.position = "bottom")
#p
if(bSaveFigures==T){
  ggsave(p,file=fnm02,
