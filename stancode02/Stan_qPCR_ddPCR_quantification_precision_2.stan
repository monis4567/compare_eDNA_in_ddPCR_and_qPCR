...
transformed parameters{
  ...
  for (i in 1:N_st_q){
    p_tmp_st[i] = exp(C_qPCR_st[i]) * -phi_0[i_qst_idx[i]];
    theta_st[i] = log1m_exp(p_tmp_st[i]) - p_tmp_st[i];
    //theta_st[i] = phi_0[i_qst_idx[i]] + (phi_1[i_qst_idx[i]] * C_qPCR_st[i]);
  }
  // // // // // Unknown (Env samples)
  vector[N_en_q] p_tmp_un;
  for (i in 1:N_en_q){
    p_tmp_un[i] = exp(C_qPCR[ij_qen_idx[i]]) * -phi_0[i_qen_idx[i]];
    theta_un[i] = log1m_exp(p_tmp_un[i]) - p_tmp_un[i];
...
model {
  ...
  // // qPCR
  // // // Bernoulli model
  Z_qPCR_st ~ bernoulli_logit(theta_st); //Standards
  Z_qPCR_en ~ bernoulli_logit(theta_un); //Unknown (Env samples)
